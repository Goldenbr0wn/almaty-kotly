...
if [[ ${#mode[@]} -gt 0 ]]; then
  cmd=(python3 "$ROOT/scripts/sgctl.py" validate-loop-design "${mode[@]}" "$1")
else
  cmd=(python3 "$ROOT/scripts/sgctl.py" validate-loop-design "$1")
fi
if "${cmd[@]}" >/tmp/sg-validate-loop.$$ 2>/tmp/sg-validate-loop-err.$$; then
  lines=$(wc -l < "$1" | tr -d ' ')
